...
public class FileLoader {
    private static final Logger LOG = Logger.getLogger(FileLoader.class.getSimpleName());
    ...
    public static FileFilter ROM_FILTER = new FileNameExtensionFilter(
            Arrays.toString(SystemProvider.psxBinaryTypes) + " files", SystemProvider.psxBinaryTypes);

    public static FileFilter SAVE_STATE_FILTER = new FileNameExtensionFilter(
            "psxsav files", new String[]{"psxsav"});

    public static void writeFileSafe(Path file, byte[] data) {
        try {
            Files.write(file, data);
        } catch (IOException e) {
            LOG.error(String.format("Unable to write file %s, #data %s", file.toAbsolutePath().toString(),
                    data.length));
        }
    }
    ...
    public static byte[] readBinaryFile(Path file, SystemProvider.SystemType systemType) {
